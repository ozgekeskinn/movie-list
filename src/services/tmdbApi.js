...
async function request(endpoint) {
  ...
  if (!response.ok) {
    throw new Error(`Film verileri alınamadı. Hata Kodu: ${response.status}`);
  }
...
export async function searchMovies(query, page = 1) {
  ...
  return request(`/search/movie?${searchParameters.toString()}`);
}
...
export async function getTopRatedMovies(page = 1) {
  ...
  return request(`/movie/top_rated?${searchParameters.toString()}`);
}
...
export async function getUpcomingMovies(page = 1) {
  ...
  return request(`/movie/upcoming?${searchParameters.toString()}`);
}
...
export async function getPopularSeries(page = 1) {
  ...
  return request(`/tv/popular?${searchParameters.toString()}`);
}

export async function getSeriesDetails(id) {
  if (!id) {
    throw new Error("Dizi ID'si bulunamadı");
  }
  ...
  return request(`/tv/${id}?${searchParameters.toString()}`);
}
